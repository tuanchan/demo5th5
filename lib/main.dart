              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    final newLanguageCode = languageCodeFromName(
                      selectedLanguage,
                    );
                                      ...
                                      itemBuilder: (_) => [
                                        ...
                                        ...courseLanguageFilters.map(
                                          (code) => PopupMenuItem(
                                            value: code,
                                            child: Text(
                                              "${languageNameFromCode(code)} • $code",
                                            ),
                                          ),
                                        ),
                                      ],
              ...
              _sectionCard(
                ...
                child: Column(
                  children: colorNames.entries
                      .map((entry) => _colorRow(entry.key, entry.value))
                      .toList(),
                ),
              ),
            ],
...
class _StatisticsPageState extends State<StatisticsPage> {
  static const int _hardCardWrongThreshold = 5;
  static const Color _dashBg = Color(0xff060a12);
  static const Color _dashPanel = Color(0xff0d1421);
  static const Color _dashPanel2 = Color(0xff121a28);
  static const Color _dashBorder = Color(0xff304267);
  static const Color _dashText = Color(0xffeef4ff);
  static const Color _dashMuted = Color(0xff93a0b5);
  static const Color _dashBlue = Color(0xff4f6dff);
  static const Color _dashPurple = Color(0xff8b65ff);
  static const Color _dashGreen = Color(0xff21c781);
  static const Color _dashOrange = Color(0xffffa31a);
  static const Color _dashRed = Color(0xffff4d57);
  ...
  Future<StatisticsData> loadStatistics() async {
    ...
    for (var i = 0; i < 7; i++) {
      ...
      dueScheduleItems.add(
        DueScheduleItem(
          label: i == 0 ? 'Hôm nay' : (i == 1 ? 'Mai' : '+$i ngày'),
          count: rows.isEmpty ? 0 : _asInt(rows.first['count']),
        ),
      );
    }
    ...
    return StatisticsData(
      ...
      reviewedTodayCards: _asInt(overview['reviewedTodayCards']),
      hardCards: _asInt(overview['hardCards']),
      srsItems: [
        SrsDistributionItem(
          label: 'Cấp 7-8',
          subtitle: 'Thành thạo',
          count: _asInt(srs['advanced']),
          color: _dashBlue,
        ),
        SrsDistributionItem(
          label: 'Cấp 4-6',
          subtitle: 'Đang ổn tốt',
          count: _asInt(srs['steady']),
          color: _dashPurple,
        ),
        SrsDistributionItem(
          label: 'Cấp 1-3',
          subtitle: 'Mới học',
          count: _asInt(srs['learning']),
          color: _dashBlue,
        ),
        SrsDistributionItem(
          label: 'Cấp 0',
          subtitle: 'Chưa thuộc / Mới',
          count: _asInt(srs['newCards']),
          color: _dashRed,
        ),
      ],
      ...
      languageItems: languageRows.asMap().entries.map((entry) {
        ...
        return LanguageDistributionItem(
          label: _languageLabelFromCode(row['languageCode']?.toString() ?? ''),
          count: _asInt(row['cardCount']),
      ...
      hardCourseItems: hardCourseRows.map((row) {
        return HardCourseItem(
          ...
          hardCards: _asInt(row['hardCards']),
      ...
      courseItems: courseRows.map((row) {
        return CourseStatisticsItem(
          ...
          needReviewCards: _asInt(row['needReviewCards']),
          reviewedTodayCards: _asInt(row['reviewedTodayCards']),
          masteredTodayCards: _asInt(row['masteredTodayCards']),
      ...
      dueItems: dueRows.map((row) {
        return ReviewDueItem(
          ...
          repetitionCount: _asInt(row['repetitionCount']),
          intervalDays: _asInt(row['intervalDays']),
        );
      }).toList(),
    );
  }
            ...
            if (wide) {
              return Column(
                children: [
                  _dashRow(
                    [
                      _buildSrsDistributionPanel(data),
                      _buildDueSchedulePanel(data),
                      _buildLanguageDistributionPanel(data),
                    ],
                    flexes: [3, 5, 4],
                  ),
                  SizedBox(height: 16),
                  _dashRow(
                    [
                      _buildOverviewPanel(data),
                      _buildMemoryChallengePanel(data),
                    ],
                    flexes: [1, 1],
                  ),
                  SizedBox(height: 16),
                  _dashRow(
                    [
                      _buildStatusRatioPanel(data),
                      _buildHardCoursesPanel(data),
                    ],
                    flexes: [5, 7],
                  ),
                ],
  ...
  Widget _buildDashboardTopBar() {
    return Row(
      children: [
        ...
        _dashIconButton(icon: Icons.refresh_rounded, onTap: reloadStatistics),
      ],
    );
  }
    ...
    return _dashCard(
      ...
      child: Column(
        children: data.srsItems.map((item) {
          final percent = _percent(item.count, data.totalCards);
          return Padding(
  ...
  Widget _buildLanguageDistributionPanel(StatisticsData data) {
    return _dashCard(
      ...
      child: data.languageItems.isEmpty
          ? _dashEmpty('Chưa có dữ liệu ngôn ngữ')
          : Row(
              children: [
                SizedBox(
                  width: 126,
    ...
    return _dashCard(
      ...
      child: Row(
        ...
        children: [
          ...
          _statusRing('Đang ôn', reviewing, data.totalCards, _dashBlue),
          _statusRing('Thẻ khó', data.hardCards, data.totalCards, _dashRed),
        ],
      ),
    );
  }
  ...
  Widget _statusRing(String label, int value, int total, Color color) {
    final percent = _percent(value, total).round();
    return Expanded(
      child: Column(
        children: [
          SizedBox(
  ...
  Widget _buildHardCoursesPanel(StatisticsData data) {
    return _dashCard(
      ...
      child: data.hardCourseItems.isEmpty
          ? SizedBox(
              height: 130,
              child: Center(child: _dashEmpty('Không có thẻ khó nào')),
            )
          : Column(
...
class _FlashCardsPageState extends State<FlashCardsPage> {
  ...
  Future<void> answerProgress({
    ...
    final studyResultId = await _insertFlashStudyResult(
      card: card,
      known: known,
    );
...
''';
  ...
  Widget buildSmallBottomIcon({
    ...
    return IconButton(
      onPressed: onTap,
      icon: Icon(icon, color: active ? Color(0xffffb020) : AppColors.border),
    );
  }
}
...
''';
  ...
  List<_GeneratedSentenceQuestion> _parseGeneratedSentenceQuestions(
    ...
    for (final item in decoded) {
      if (item is! Map) continue;
      final cardId = int.tryParse(item['cardId']?.toString() ?? '');
      final question = _cleanGeneratedSentence(
...
''';
  ...
  Future<void> _answerCard(StudyCardItem card, String selected) async {
    ...
    final isCorrect =
        _normalizeAnswer(selected) == _normalizeAnswer(correctText);
        ...
        return StatefulBuilder(
          builder: (context, setSheetState) {
            ...
            final promptText = _listening
                ? wrongCard.term.trim()
                : _promptOf(wrongCard).trim();
            final correctAnswer =
                (_listening ? wrongCard.definition : _answerOf(wrongCard))
                    .trim();
  ...
  Future<void> _submitMultipleChoice() async {
    ...
    for (final card in skippedCards) {
      await _recordStudyResult(card: card, answerText: '', isCorrect: false);
    }
  ...
  Future<void> _submitEssay({bool allowEmptyAsSkip = false}) async {
    ...
    final ok = _sentenceMode
        ? _isSentenceAnswerCorrect(card, typed)
        : _isEssayAnswerCorrect(card, typed);
    ...
    if (!_usesGeminiTextGrading) {
      await _recordStudyResult(card: card, answerText: typed, isCorrect: ok);
    }
  ...
  Future<void> _submitListeningAnswer() async {
    ...
    final card = _quizCards[_currentEssayIndex];
    final ok = _acceptedListeningAnswersOf(
      card,
    ).contains(_normalizeAnswer(selected));
    final wasLast = _currentEssayIndex + 1 >= _quizCards.length;
    ...
    await _recordStudyResult(card: card, answerText: selected, isCorrect: ok);
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        SizedBox(width: 10),
                        Expanded(
                          child: _resultBox('Sai', '$_wrong', AppColors.red),
                        ),
                        SizedBox(width: 10),
                        Expanded(
                          child: _resultBox('Tổng', '$_total', AppColors.blue),
                        ),
                      ],
                    ),
                    SizedBox(height: 14),
  ...
  Widget _buildMultipleChoiceMode() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) =>
          _buildQuestionCard(_quizCards[index], index),
    );
  }
